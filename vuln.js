...
// 3. ReDoS - use a static, safe email regex instead of building from user input
...
async function fetchUserData(url) {
  ...
  const allowedHosts = (process.env.ALLOWED_FETCH_HOSTS || '').split(',').map(h => h.trim()).filter(Boolean);
